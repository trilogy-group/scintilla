#!/bin/bash
# Scintilla EC2 User Data Script
# Automatically install and configure Scintilla on EC2 instances
...
echo "Starting Scintilla installation at $(date)"
    ...
    "logs": {
        "logs_collected": {
            "files": {
                "collect_list": [
                    {
                        "file_path": "/var/log/scintilla/*.log",
                        "log_group_name": "/aws/scintilla/application",
                        "log_stream_name": "{instance_id}/application",
                        "timezone": "UTC"
                    },
                    {
                        "file_path": "/var/log/nginx/scintilla_access.log",
                        "log_group_name": "/aws/scintilla/access",
                        "log_stream_name": "{instance_id}/nginx-access",
                        "timezone": "UTC"
                    },
                    {
                        "file_path": "/var/log/nginx/scintilla_error.log",
                        "log_group_name": "/aws/scintilla/access",
...
sudo cat > /etc/logrotate.d/scintilla << 'EOF'
/var/log/scintilla/*.log {
    daily
    missingok
    rotate 30
    compress
    delaycompress
    notifempty
    create 644 scintilla scintilla
    postrotate
        systemctl reload scintilla || true
    endscript
}
EOF
...
sudo cat > /opt/scintilla/scripts/startup.sh << 'EOF'
#!/bin/bash
# Startup script for Scintilla

echo "Starting Scintilla services..."

# Initialize database
/opt/scintilla/init_db.sh
...
# Start Scintilla
systemctl start scintilla
systemctl enable scintilla

echo "All services started successfully"
EOF

sudo chmod +x /opt/scintilla/scripts/startup.sh
